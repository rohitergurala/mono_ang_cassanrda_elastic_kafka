...
class MonoAngCassElasticKafkaKafkaResourceIT {
    ...
    void setup() {
        ...
        MonoAngCassElasticKafkaKafkaResource kafkaResource = new MonoAngCassElasticKafkaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mono-ang-cass-elastic-kafka-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mono-ang-cass-elastic-kafka-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
